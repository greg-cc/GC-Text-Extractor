...
from tkinter import (Label, Frame, IntVar, BooleanVar, DoubleVar, Scale, Entry, Checkbutton, Spinbox, Button, Text, Scrollbar, PanedWindow,
                     SUNKEN, W, X, Y, BOTTOM, LEFT, TOP, BOTH, HORIZONTAL, RIGHT, NW, DISABLED, NORMAL, END, RAISED, VERTICAL) # Ensured VERTICAL is here
import os
import re
import json
import traceback
...
APP_VERSION = "1.5.2" # Two-column layout for settings

# --- Default values (no change from 1.4.9 / 1.5.1 logic) ---
DEFAULT_MIN_WORDS_GENERAL = 11; DEFAULT_MIN_WORDS_SENTENCE = 5; DEFAULT_ALPHANUM_ENABLED = 1; DEFAULT_ALPHANUM_THRESHOLD = 0.75
DEFAULT_REMOVE_CODE_BLOCKS = 1; DEFAULT_REMOVE_CONCAT_ENTIRELY = 1; DEFAULT_REMOVE_SYMBOL_ENCLOSED = 1
DEFAULT_MIN_CODE_KEYWORDS = 1; DEFAULT_MIN_CODE_SYMBOLS = 2; DEFAULT_MIN_WORDS_CODE_CHECK = 2; DEFAULT_CODE_SYMBOL_DENSITY = 0.20
DEFAULT_MIN_LEN_CONCAT_CHECK = 18; DEFAULT_MIN_SUB_WORDS_REPLACE = 3; DEFAULT_MAX_SYMBOLS_AROUND = 3

CODE_KEYWORDS_LIST = { # Unchanged
        ...
        if os.path.exists(SETTINGS_FILENAME):
            ...
            for var_name, value in loaded_settings.items():
                ...
                if var_name in SETTINGS_CONFIG and var_object:
                    ...
                    except Exception: print(f"WARN: Could not set {var_name} from settings. Using default.")
...
def save_app_settings(): # Unchanged
    ...
    if not settings_to_save: print("WARN: No settings to save."); return
    try:
...
def create_synchronized_setting(parent, label_text, var, from_, to, resolution=None, is_int=True, label_width=26, control_length=180):
    ...
    return frame
def create_spinbox_setting(parent, label_text, var, from_, to, label_width=24, spinbox_width=5, indent=15):
    ...
    return spinbox
def create_scale_setting(parent, label_text, var, from_, to, resolution, label_width=24, scale_length=150, indent=15):
    frame = Frame(parent); frame.pack(side=TOP, fill=X, padx=5, pady=1)
    Label(frame, text=label_text, width=label_width, anchor=W).pack(side=LEFT, padx=(indent, 5))
...
# --- MODIFIED: populate_settings_content for two columns ---
def populate_settings_content(parent_scrollable_frame):
    """Populates the given frame with all filter setting controls in two columns."""
    setup_variables() 
    load_app_settings()

    # Main container for the two columns within the scrollable area
    column_container = ttk.Frame(parent_scrollable_frame)
    column_container.pack(fill=BOTH, expand=True)

    # Left Column
    left_column = ttk.Frame(column_container, padding=(0,0,10,0)) # Add some padding to the right of left column
    left_column.pack(side=LEFT, fill=Y, expand=False, anchor=NW) # Fill Y, don't expand X for left

    # Right Column
    right_column = ttk.Frame(column_container)
    right_column.pack(side=LEFT, fill=BOTH, expand=True, anchor=NW) # Fill Y and expand X for right

    # --- Left Column Content: Basic Filters ---
    Label(left_column, text="Basic Filters:", font=('Helvetica', 10, 'bold')).pack(side=TOP, pady=(5,2), anchor=NW, padx=5)
    ...
    # --- Right Column Content: Advanced Filters ---
    Label(right_column, text="Advanced Word/Block Filters:", font=('Helvetica', 10, 'bold')).pack(side=TOP, pady=(5,2), anchor=NW, padx=5)
    
    # Code Block Filter
    code_filter_frame_right = Frame(right_column); code_filter_frame_right.pack(side=TOP, fill=X, padx=5, pady=2) # Use specific parent
    ...
    concat_filter_frame_right = Frame(right_column); concat_filter_frame_right.pack(side=TOP, fill=X, padx=5, pady=2) # Use specific parent
    ...
    symbol_filter_frame_right = Frame(right_column); symbol_filter_frame_right.pack(side=TOP, fill=X, padx=5, pady=2) # Use specific parent
...
# --- Text Extraction and Processing Logic (no changes from 1.4.6 / 1.4.9 logic) ---
def extract_text_from_txt(filepath): # ... (as in v1.4.9)
    try:
...
def extract_text_from_docx(filepath): # ... (as in v1.4.9)
    try:
...
def extract_text_from_pdf(filepath): # ... (as in v1.4.9)
    text = "";
    try:
    ...
    return text
def get_alphanumeric_ratio(text_segment): # ... (as in v1.4.9)
...
def is_sentence_or_long_sequence(text_segment, min_words_general_sequence=6, min_words_punctuated_sentence=2): # ... (as in v1.4.9)
    ...
    return False
def split_concatenated_token(token): # ... (as in v1.4.9)
    if not token: return []
...
def is_code_like_segment(segment_text, words_in_segment, 
                         min_keywords, min_symbols, min_words_check, symbol_density_thresh): # ... (as in v1.4.9)
...
def process_text(full_text, min_words_general, min_words_sentence, 
                 ...
                 do_remove_code_blocks, code_min_kw, code_min_sym, code_min_words_seg, code_sym_dens): # ... (as in v1.4.9)
    extracted_content = []
    ...
    for segment_idx, segment in enumerate(segments):
        ...
        if modified_segment.strip(): extracted_content.append(modified_segment)
    return "\n\n".join(extracted_content)

# --- Filter Test Pad UI Population (unchanged from 1.5.1 logic) ---
def populate_test_pad_ui(parent_frame): # ... (as in v1.5.1)
    ...
    input_pane = Frame(pw, relief=SUNKEN, borderwidth=1) # Use tk.Frame
    ...
    input_text_sb.config(command=g_test_pad_input_text.yview)
    input_text_sb.pack(side=RIGHT, fill=Y)
    g_test_pad_input_text.pack(side=LEFT, fill=BOTH, expand=True)
    pw.add(input_pane, stretch="always", width=350)
    output_pane = Frame(pw, relief=SUNKEN, borderwidth=1) # Use tk.Frame
    ...
    output_text_sb.config(command=g_test_pad_output_text.yview)
    output_text_sb.pack(side=RIGHT, fill=Y)
    g_test_pad_output_text.pack(side=LEFT, fill=BOTH, expand=True)
    pw.add(output_pane, stretch="always", width=350)
...
# --- process_pasted_text (unchanged from 1.5.1 logic) ---
def process_pasted_text(): # ... (as in v1.5.1)
    global g_test_pad_input_text, g_test_pad_output_text
...
# --- File Processing Orchestration (unchanged) ---
def process_file(filepath): # ... (as in v1.4.9)
    global status_label
    if not os.path.exists(filepath): status_label.config(text=f"Error: File not found {filepath}"); return
    filename_base, extension = os.path.splitext(filepath); extension = extension.lower()
    full_text = ""; status_label.config(text=f"Processing: {os.path.basename(filepath)}..."); root.update_idletasks()
    if extension == '.txt': full_text = extract_text_from_txt(filepath)
    elif extension == '.docx': full_text = extract_text_from_docx(filepath)
    elif extension == '.pdf': full_text = extract_text_from_pdf(filepath)
    else: status_label.config(text=f"Unsupported file type: {extension} for {os.path.basename(filepath)}"); return
    ...
    output_filepath = filename_base + "_txtextract123.txt"
    try:
...
def drop_handler(event): # ... (as in v1.4.9)
...
root.title(f"File Text Extractor v{APP_VERSION}"); root.geometry("800x750") # Default size
...
# --- Section 1: Filter Settings (Scrollable) ---
settings_container_frame = Frame(root, relief=SUNKEN, borderwidth=1) # Outer container for the title + scroll area
settings_container_frame.pack(side=TOP, fill=X, padx=7, pady=(7,0))
...
settings_scroll_canvas_frame = Frame(settings_container_frame)
settings_scroll_canvas_frame.pack(fill=X, expand=False) # Don't let canvas frame expand vertically indefinitely

settings_canvas = tk.Canvas(settings_scroll_canvas_frame, borderwidth=0, height=280) # Fixed height for settings area
settings_scrollbar = ttk.Scrollbar(settings_scroll_canvas_frame, orient="vertical", command=settings_canvas.yview)
scrollable_settings_content_frame = ttk.Frame(settings_canvas) # Content goes here
...
settings_canvas.create_window((0, 0), window=scrollable_settings_content_frame, anchor="nw", tags="settings_content_window")
settings_canvas.configure(yscrollcommand=settings_scrollbar.set)

# Make content frame expand to width of canvas
def _configure_settings_content_frame(event):
    canvas_width = event.width
    settings_canvas.itemconfig("settings_content_window", width=canvas_width)
settings_canvas.bind("<Configure>", _configure_settings_content_frame)


settings_canvas.pack(side=LEFT, fill=X, expand=True)
settings_scrollbar.pack(side=RIGHT, fill=Y)

populate_settings_content(scrollable_settings_content_frame) # Populate into the inner frame

# --- Section 2: Filter Test Pad ---
test_pad_container_frame = Frame(root, relief=SUNKEN, borderwidth=1) # Outer container
test_pad_container_frame.pack(side=TOP, fill=BOTH, expand=True, padx=7, pady=7)
Label(test_pad_container_frame, text="Filter Test Pad", font=('Helvetica', 12, 'bold')).pack(anchor=W, padx=5, pady=(5,0))
populate_test_pad_ui(test_pad_container_frame) # Pass the container

# --- Section 3: File Processing (Drag & Drop) ---
file_processing_container_frame = Frame(root, relief=SUNKEN, borderwidth=1)
file_processing_container_frame.pack(side=TOP, fill=X, padx=7, pady=(0,7))
...
print("--- Main GUI Setup Complete. Starting mainloop. ---")
root.mainloop()